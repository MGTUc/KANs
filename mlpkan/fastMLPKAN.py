...
class FastMLPKANlayer(nn.Module):
...
class FastMLPKAN(nn.Module):
    def __init__(self, layerSizes, subnetwork_shape = [2,2]):
        super(FastMLPKAN, self).__init__()
        ...
        for i in range(len(layerSizes)-1):
            mlpkan_layer = FastMLPKANlayer(input_size=layerSizes[i], output_size=layerSizes[i+1], subnetwork_hidden_shape=self.subnetwork_shape)
...
if __name__ == "__main__":
    ...
    MLPKANmodel = FastMLPKAN([2,3,1], subnetwork_shape=[10,5])
